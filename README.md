...
Alongside the probability bar, a one-sentence **narrative** gives human-readable context — e.g. *"Most TAMA38 projects in {neighbourhood} reach Form 4 within about X years of Form 1 — at this stage, that typically leaves ~Y more years."*  It prefers the building's own neighbourhood (if it has ≥ 5 completed projects to give a stable median) and falls back to the citywide median otherwise.

### TAMA38 Likelihood Dashboard
For buildings without an existing TAMA38 permit, the app scores the building across several heuristic signals, **plus** an independent ML estimate:
...
- **ML Likelihood Estimate** — a `HistGradientBoostingClassifier` trained citywide on every address's age, floor count, nearby permit density, open-site flag, and neighbourhood, labelled by whether that address already has a TAMA38 permit.  It surfaces the same signals as the heuristic score but learned from data rather than hand-tuned weights, so the two can be sanity-checked against each other.
...
├── ml_score.py             # ML completion forecast + likelihood estimate (loads both .pkl bundles)
├── train_tama_model.py     # Trains both HistGradientBoosting classifiers (completion + likelihood)
...
├── requirements.txt
├── tama_model.pkl                    # Completion-forecast model bundle (generated by train_tama_model.py)
├── tama_model_meta.json              # Human-readable training metadata
├── tama_likelihood_model.pkl         # Citywide likelihood model bundle
├── tama_likelihood_model_meta.json   # Human-readable training metadata
├── data/
...
| `GET` | `/api/search?street=&building=` | Building geometry + TAMA38 analysis + ML completion forecast (with narrative) or ML likelihood estimate |
...
`train_tama_model.py` trains two independent `HistGradientBoostingClassifier` models (scikit-learn, natively handles missing values) in a single run.

### 1. Completion model (`tama_model.pkl`)

For buildings that already have an active TAMA38 permit — probability of reaching Form 4.
...
6. Fit on full dataset and save `tama_model.pkl`, along with per-neighbourhood duration stats (`neighborhood_stats`) used to phrase the completion-forecast narrative shown in the UI
...
### 2. Likelihood model (`tama_likelihood_model.pkl`)

For buildings that **don't** have a TAMA38 permit yet — how "TAMA38-like" the building looks, based only on signals available for every address citywide: building age, floor count, permit density within 200 m / 500 m, an open-construction-site flag, and neighbourhood.

**Training flow:**
1. Query every address in `TLV.addresses` (52k) with its building age/floors, nearby permit counts, open-site flag, and neighbourhood — all computed in SQL against the same tables the live app queries
2. Label = 1 if that address already has a TAMA38-flagged permit (~2,943 addresses), else 0
3. Fit with `class_weight="balanced"` to handle the ~5.6% positive rate; cross-validate the same way as the completion model
4. Save `tama_likelihood_model.pkl` + `tama_likelihood_model_meta.json`

This gives the Likelihood Dashboard a data-driven estimate to sit alongside the hand-tuned heuristic score — both are shown so they can be cross-checked against each other.
...
- [x] Citywide ML likelihood model for buildings without an existing TAMA38 permit
- [x] Human-readable narrative alongside the ML completion forecast (neighbourhood/citywide typical duration)
